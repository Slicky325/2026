...
In this equation, $S_i$ denotes the subset of token indices currently retained in the KV cache constrained by budget $k$ (where $|S_i| \le k$), and $D_i$ is the scalar normalization factor computed by summing the exponential attention scores of the keys present in $S_i$.
